...
合成滑鼠/鍵盤時，**按下與放開不要放在同一幀**。`update()` 在 `poke()` 之後才跑，
同幀放開等於 `mouse.down` 從來沒有為真過——讀「這一幀有沒有按著」的程式碼（序章的
`shoot` 節拍就是）永遠不會觸發，整個情境停在那一格，看起來全綠但一幕都沒演完。

探針要斷言**那件事本身**，不要斷言「相關現象」。擊殺噴發那條第一版寫成
「怒火往上跳而且場上有噴發物」——命中回怒每一刀都在跳，條件被普通攻擊滿足，
把整段吸收邏輯刪掉還是全綠。改成看 `moteGrabs` 計數器才真的紅。
**每加一條探針，動手把它守的東西弄壞一次，確認它會紅。** 這個專案已經靠這招抓到四次假綠。

改語意時要重新檢查探針的判準。整備從「裝上＝袋子少一件」改成「置換＝換下的回袋子」之後，
袋子長度是**不變**的——舊斷言於是永遠成立。語意變了，判準也要跟著變。

改函式簽章時，**先 grep 全部呼叫點再改**。`startSwing(u, fireRateMult)` 改成吃招式之後，
序章那個腳本化的一刀沒跟著改，`1` 被當成招式傳進去 → 三段時間全變 NaN →
`s.t >= aEnd + s.recover` 對 NaN 恆為 false → `u.swing` 永遠不清 →
之後每次 `startSwing` 都被開頭的閘門擋掉＝**玩到一半突然再也揮不動**，
而且沒有例外、沒有紅字。JS 不會幫你擋這種事，探針才會（見 `noStuckSwing`）。
...
5. **不引入依賴、不破壞單檔開檔即玩。**
6. **騎士身上沒有人間的甲**（§2.6c）。`armorBase` 恆為 0，他唯一的防護是結晶疊出來的
   `armorTemp`。裝甲只有一條算式、兩個來源（敵人天生穿著／騎士臨時結晶）。
7. **一般擊殺不噴聖血**（§6.2）。血是處決專屬——兩者等價的話，體幹→踉蹌→處決那整條鏈
   就變成可選的，而那條鏈是整個資源迴圈的心臟。
8. **跨房不回血**（`interMissionHealFrac = 0`）。太硬的話補償的旋鈕是**處決的回血量**，
   不是把跨房回血加回來。
...
- **位置式解碰 + 大位移＝穿牆。** `pushOutSeg` 只看「現在有沒有插進牆裡」，沒有掃掠判定。
  一幀跨過整道薄牆時它會在**另一側**把你推得更遠＝穿過去卡在柱子後面。
  所有大位移（擊退、dash）一律走 `stepCollide()`：分段推進、每段不超過半個身體半徑、
  每段結束解一次牆碰。
- **非 COMBAT 的計時不能放在 `update()`**（那支只跑 COMBAT）。下墜整備的倒數掛在主迴圈的
  `frame()` 上——**harness 因此也要另外推 `tickDescent`**，不然相關探針會永遠量不到東西。
- **`u.weapon` 是 `WEAPONS` 的共用參照，絕對不可以改。** 要改一律用 per-unit 旋鈕
  （`pierceBonus` / `reachMul` / `knockMul` / `poiseAdd` / `dashCdMul` …）。
  結晶、誓約、聖物全部走這條路。
...
1. 讀 `DESIGN.md`（設計是什麼）。**特別是 §2.2/§2.3 怒火、§2.4 處決、§2.6b 主副手、
   §2.6c 裝甲與結晶、§5.4b 下墜整備、§6.2 兩層分離**——這幾節是最近幾輪長出來的，
   而且互相咬得很緊，改一個常常會弄壞另一個。
2. 讀 `HARVEST.md` §E 的「建議順序」與「玩測輪次」（做到哪、下一步是什麼）
