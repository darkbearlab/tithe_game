...
> ⚠️ **調值待辦（D2/D3 期間反覆撞到）**：以現在的數值，敵人**常常在體幹破之前就先被血量殺死**——
> 實測 shieldman 體幹只掉到 38/60 就死了；試作場的三隻非假人也在一分鐘內被清光。
> 處決是唯一的回血手段，窗口卻很少出現＝迴圈接不起來。
> 要嘛提高 `poiseDmg`、要嘛降低體幹上限、要嘛拉高敵人 HP（`DESIGN.md` §2.8 推論 3 說威脅該來自
> 傷害與速度而非血量，所以傾向前兩者）。**這條要等玩測，不要憑空調。**
> harness 的處決探針因此**直接塞 `staggerT`** 跳過整條數值鏈——它驗的是機制，不是平衡。
...
| **0** | **主副手＝左右鍵**（moveset 改造） | — | **下一項**。武器從「一組揮擊參數」變成「一袋招式」；`startSwing(u)` → `startSwing(u, move)`。招架掛到副手槽，副手四選一變成真取捨表。連帶刪 `X` 換武器組／`CONFIG.weaponSwap`／`slotsOf` 2 格規則／`weight` 欄位。見 `DESIGN.md` §2.6b。**排在 D6 之前**——整備畫面要先知道「一套配裝是什麼」 |
...
| 6 | 🔸 **房間流程**（做了第一塊） | C6 ✅ | ~~清場 → 中央開洞 → 跳下去~~ **已完成**：`CONFIG.exit`、`openExitPortal()`（開在離房間中央最近的 waypoint＝保證站得到）、碎裂石環 + 黑洞 + 光柱、邊緣指標、清場提示；**走進洞才過關**。<br>**還沒做**：進門鎖門＋全員 ENGAGED、洞口的**三選一全揭露**、層與房間的串接。<br>**前置**：程序生成房間要能自動產 waypoint 圖（把編輯器的「鋪滿網格點 + 自動整理連線」搬進遊戲端） |
...
| **2** | **重量掉落 bug + 體幹/處決平衡** | 小 | `updateDrops()` 掉 `quality`/`enh`（§6.3）；以及那個一直懸著的「敵人在體幹破之前就先死」。**重擊本身就是開處決窗的工具**，所以要等步驟 1 落地才量得準 |
