...
- **數量用機率限制，不設硬上限**。偶爾滾出一把六詞條的怪物＝roguelite 的故事時刻。
- **但要分兩池**：
  | | 數量 | 理由 |
  |---|---|---|
  | **數值型**（+傷害／+體幹傷／+破甲…） | 機率，**不設上限** | 疊起來只是更強，尾巴很香 |
  | **效果型/觸發型**（投射物／AOE／雷射） | **每把 1~2** | 它是這把武器的**身分**。兩個以上就沒有身分，只有雜訊 |
- 效果型掛在**「你打對了」的瞬間**（處決／招架／重擊／體幹破），**不要掛在每一刀**——
  掛在每一刀的話這個遊戲會從近戰讀招變成彈幕自動戰鬥。
- **`quality` 要改義**：現在是「charge 格數預算」（`WEAPON_QUALITY`，鐵匠在用），
  提案改成**機率倍率**——精良度越高，每一輪詞條擲骰越容易成功。一個概念兩個用途，
  不然掉落走機率、鐵匠走格數會變成兩套系統。
- 效果型詞條和結晶**共用同一套效果系統**（Hooks）。差別只是掛在武器上還是身上。
